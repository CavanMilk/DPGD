...
class RMSEMeter:
    ...
    def update(self, data, preds, truths, mae, eval=False):
        ...
        mae = metrics.mean_absolute_error(preds[0, 0, :], truths[0, 0, :])
    ...
    def report(self):
        return f'RMSE = {self.measure():.6f}, MAE = {self.mae():.6f}'
...
class Trainer(nn.Module):
    ...
    def test(self, loader, save_path=None):
        ...
        if save_path is None:
            save_path_res = os.path.join(self.workspace, 'results')
            save_path = os.path.join(save_path_res, f'{self.name}_{self.args.dataset}_{self.args.scale}')
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            if self.local_rank == 0:
                for metric in self.metrics:
                    metric.update(data, preds, truths, loss.item())
    ...
    def evaluate_one_epoch(self, loader):
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                if self.local_rank == 0:
                    for metric in self.metrics:
                        metric.update(data, preds, truths, loss.item(), eval=True)
                    pbar.set_description(f'loss={total_loss[-1]:.4f}')
                    pbar.update(loader.batch_size * self.world_size)
    ...
    def save_checkpoint(self, full=False, best=False):
        ...
        if full:
            ...
            state['amp'] = self.model.state_dict()
